# Copilot Instructions for `agent-ops`

## Copilot-specific usage notes

- Keep edits scoped to the relevant package under `packages/`; avoid cross-package internal imports.
- Preserve the standard package scaffold (`src/index.ts`, `src/types.ts`, `tests/<name>.test.ts`, `tsconfig.json` extending `../../tsconfig.base.json`).
- Use workspace commands from repo root (`pnpm build`, `pnpm test`, `pnpm lint`) and package-local commands only when targeting one module.
- Keep module public APIs exported from `src/index.ts`; define shared contracts in `src/types.ts`.
...
cd packages/action-ledger && pnpm test -- tests/action-ledger.test.ts  # run one test file
